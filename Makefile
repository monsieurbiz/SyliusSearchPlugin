...
DOCTRINE_NAMESPACE=MonsieurBiz\SyliusSearchPlugin\Migrations
...
sylius: dependencies sylius.database sylius.fixtures sylius.assets sylius.elasticsearch ## Install Sylius
.PHONY: sylius
...
sylius.elasticsearch: ## Setup the database
...
docker.destroy: ## Destroy the containers, volumes, networks…
	cd ${APP_DIR} && ${COMPOSE} down -v --remove-orphan
